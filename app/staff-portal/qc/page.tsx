...
export default async function StaffQcPage({
  ...
  const jobCards = jobs.map((job) => {
    const returnOpen = job.stages.some(
      (stage) => stage.key === 'return_quality_check' && ['open', 'in_progress'].includes(stage.status),
    );
    const packingOpen = job.stages.some(
      (stage) => stage.key === 'packing' && ['open', 'in_progress'].includes(stage.status),
    );
    return {
      id: job.id,
      ...
      departmentStatus: returnOpen ? 'Return QC' : packingOpen ? 'Packing' : view === 'closed' ? 'QC completed' : 'Quality check',
  ...
  return (
    ...
    >
      ...
      {selectedJobId ? <QcJobModal jobId={selectedJobId} view={view} /> : null}
    </StaffPortalShell>
  );
}
