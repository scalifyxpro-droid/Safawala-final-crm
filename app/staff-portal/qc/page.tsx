...
export default async function StaffQcPage({
  ...
  const { view: requestedView, job: selectedJobId, q = '', sort = 'event', eventDate = '', bookingDate = '' } = params as typeof params & { q?: string; sort?: string; eventDate?: string; bookingDate?: string };
                    ...
                    <ul className="divide-y divide-border">
                      {dateJobs.map((job) => {
                        const returnOpen = job.stages.some(
                          (stage) =>
                            stage.key === 'return_quality_check' &&
