...
import { Download, Eye, FileText, LoaderCircle, Pencil, Plus, Trash2, X } from 'lucide-react';
...
// Branded expense report: header banner (crown logo), a bordered table of the
// currently filtered rows and the same footer treatment as every other
// generated document, replacing the previous window.print() "PDF" button.
async function downloadExpensesReportPdf(rows: FinanceRecord[], columns: string[][]) {
  ...
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(12);
  doc.text(`${rows.length} record${rows.length === 1 ? '' : 's'}`, right, headerTop + 10, { align: 'right' });
  doc.setFontSize(8.5);
  doc.setFont('helvetica', 'normal');
  doc.text('EXPENSES REPORT', right, headerTop + 17, { align: 'right' });
  doc.setFontSize(7.5);
  doc.setTextColor(...MUTED);
  doc.text(`Generated ${generatedOn}`, right, headerTop + 23, { align: 'right' });

  let y = headerTop + headerHeight + 8;
  const columnWidth = boxWidth / (columns.length + 1);

  const tableHeader = () => {
    doc.setFillColor(245, 245, 245);
    doc.setDrawColor(...BORDER_SOFT);
    doc.setLineWidth(0.3);
    doc.rect(left, y, boxWidth, 7, 'FD');
    doc.setTextColor(...BRAND_DARK);
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(8);
    columns.forEach(([, label], index) => doc.text(label.toUpperCase(), left + 3 + columnWidth * index, y + 4.8));
    doc.text('AMOUNT', left + 3 + columnWidth * columns.length, y + 4.8, { align: 'left' });
    y += 7;
  };
  ...
  rows.forEach((row, index) => {
    ...
    if (index % 2 === 0) {
      ...
      doc.rect(left, y, boxWidth, 7, 'F');
    }
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8);
    doc.setTextColor(...BRAND_DARK);
    columns.forEach(([key], index2) => {
      const clipped = doc.splitTextToSize(textValue(row[key]) || '-', columnWidth - 4)[0] || '-';
      doc.text(clipped, left + 3 + columnWidth * index2, y + 4.8);
    });
    doc.text(`Rs. ${amount.toLocaleString('en-IN')}`, left + 3 + columnWidth * columns.length, y + 4.8);
    y += 7;
  ...
  if (!rows.length) {
    ...
    doc.setFontSize(9);
    doc.setTextColor(...MUTED);
    doc.text('No records match the current search.', left + 2, y + 8);
  } else {
    y += 5;
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(9.5);
    ...
    doc.text(`Rs. ${total.toLocaleString('en-IN')}`, right, y, { align: 'right' });
  }

  stampFooterOnAllPages(doc, { left, right, note: 'Generated by Safawala.' });
  doc.save(`safawala-expenses-${new Date().toISOString().slice(0, 10)}.pdf`);
}
