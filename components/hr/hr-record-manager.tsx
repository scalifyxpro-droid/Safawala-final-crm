...
const fields: Record<string, { name: string; label: string; type?: string }[]> =
  {
    ...
    payroll: [
      { name: 'period', label: 'Payroll month', type: 'date' },
    ...
    letters: [
      { name: 'letter_type', label: 'Letter type' },
    ...
    kyc: [
      ...
      { name: 'address_proof', label: 'Address proof' },
      { name: 'bank_details_status', label: 'Bank details status' },
    ],
    'work-orders': [
      { name: 'title', label: 'Work order' },
      { name: 'department', label: 'Department' },
      { name: 'due_date', label: 'Due date', type: 'date' },
    ],
  };

export function HrRecordManager({
  module,
  staff,
}: {
  module: string;
  ...
  function submit(form: HTMLFormElement) {
    ...
    start(async () => {
      ...
      const { error } = await insertHrRecordAction(module, payload);
      setMessage(error || 'Record saved successfully.');
      if (!error) {
        setOpen(false);
        form.reset();
      ...
      <CardContent className="flex items-center justify-between gap-4 p-5">
        <div>
          <p className="font-semibold">
            Add{' '}
            {module === 'work-orders' ? 'work order' : module.replace('-', ' ')}
          </p>
          <p className="text-sm text-muted-foreground">
            Create a record for an existing staff member.
          </p>
          {message && <p className="mt-2 text-sm text-[#70481c]">{message}</p>}
        </div>
                ...
                >
                  <label className="block text-sm font-medium">
                    ...
                    <select
                      name="staff_id"
                      required
                      className="mt-1 h-10 w-full rounded-lg border bg-white dark:bg-card px-3"
                    >
                      {staff.map((person) => (
