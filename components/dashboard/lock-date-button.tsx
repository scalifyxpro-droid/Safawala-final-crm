...
export function LockDateButton({ dates, loadError = false }: { dates: LockedDate[]; loadError?: boolean }) {
  ...
  const modal = open ? <div role="presentation" className="fixed inset-0 z-[100] flex items-center justify-center overflow-y-auto bg-black/50 p-3 backdrop-blur-[1px] sm:p-6" onMouseDown={(event) => { if (event.target === event.currentTarget && !busy) setOpen(false); }}>
      <div role="dialog" aria-modal="true" aria-labelledby="lock-date-title" className="my-auto flex max-h-[calc(100dvh-1.5rem)] w-full max-w-xl flex-col overflow-hidden rounded-2xl border border-[#e4d2b6] bg-white shadow-2xl dark:border-[#493822] dark:bg-card sm:max-h-[min(88dvh,780px)]">
        <div className="flex shrink-0 items-start justify-between gap-4 border-b border-border bg-[#fcfaf7] px-4 py-4 dark:bg-[#241e17] sm:px-6">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-4 sm:p-6">
          ...
          <div className="mt-5"><h3 className="text-sm font-semibold">Upcoming locked dates <span className="text-muted-foreground">({dates.length})</span></h3>
            <div className="mt-2 divide-y divide-border overflow-hidden rounded-xl border border-border">
              {dates.length ? dates.map((entry) => <div key={entry.id} className="flex min-w-0 items-start gap-3 p-3 sm:items-center"><span className="grid size-9 shrink-0 place-items-center rounded-lg bg-[#f5ead8] text-[#70481c] dark:bg-[#33291c]"><LockKeyhole className="size-4" /></span><div className="min-w-0 flex-1"><p className="break-words text-sm font-medium">{entry.label}</p><p className="text-xs text-muted-foreground">{String(entry.locked_date)}{entry.notes ? ` · ${entry.notes}` : ''}</p></div><Button type="button" variant="ghost" size="icon-sm" aria-label={`Remove ${entry.label} on ${String(entry.locked_date)}`} title="Remove locked date" disabled={busy} onClick={() => void removeDate(entry.id)}><Trash2 className="size-4 text-destructive" /></Button></div>) : <p className="p-4 text-sm text-muted-foreground">No upcoming locked dates.</p>}
            </div>
          </div>
        </div>
      </div>
